...
int get_eid_channel_id_ok(int nid0, int nid1, int channel_id, unordered_set<int>& baned_edges) {
    ...
    int res = -1;
    const auto& eids = nodes[nid0].eids;
    int beg = rand() % eids.size();
    for (int i = 0; i < eids.size(); ++i) {
        int eid = eids[(i + beg) % eids.size()];
        if (baned_edges.count(eid)) continue;
        auto& edge = edges[eid];
...
bool pave() {
    ...
    vector<int> pids_2;// 属于2路径业务的路径
    vector<int> pids_non2;// 属于非2路径业务的路径

    for (auto& op : operations) {
        if (op.paths.size() != 2) {
            for (auto& pid : op.paths) {
                pids_non2.push_back(pid);
            }
            continue;
        } else {
            for (auto& pid : op.paths) {
                pids_2.push_back(pid);
            }
            continue;
        }
...
//            }
    ...
    // 1. 铺设2路径业务的路径
    shuffle(pids_2.begin(), pids_2.end(), g);
    ...
    shuffle(pids_non2.begin(), pids_non2.end(), g);

    for (auto& pid : pids_non2) {
