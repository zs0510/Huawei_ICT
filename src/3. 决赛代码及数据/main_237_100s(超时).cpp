...
static const double process_time_max = 100.0;  // 设定程序运行的最长时间(秒)
...
//        }
        ...
        if (jumps_min[edges[i].nid0].count(edges[i].nid1)) {
            if (edges[i].jump < edges[jumps_min[edges[i].nid0][edges[i].nid1]].jump
                || ((edges[i].jump == edges[jumps_min[edges[i].nid0][edges[i].nid1]].jump)
                    && (edges[i].distance <= edges[jumps_min[edges[i].nid0][edges[i].nid1]].distance))) {
