...
//            }
            ...
            vector<int> path_nodes_tmp_shortest;
            vector<vector<int>> path_edges_total_tmp_shortest;
            for (int channel_id = 0; channel_id < num_of_channels; ++channel_id) {
                ...
                if (!path_nodes_tmp.empty()) {
                    // TODO: 考虑使用尽可能少的边去? 而不是遇到第一个就 break
                    // 目前有 bug: 不 break 会出现通道使用冲突
                    ...
                    if (is_channel_id_ok(path_edges_total_tmp, channel_id, path_edges)) {
                        ...
                        if (path_channel_id == -1 || path_nodes_tmp.size() < path_nodes_tmp_shortest.size()) {
                            ...
                            path_edges_total_tmp_shortest = path_edges_total_tmp;
                            break;
                        }
                    }
                }
            ...
            if (path_channel_id != -1) {
                ...
                path_edges_total = path_edges_total_tmp_shortest;
            }
        ...
        // 请保证 path_edges 及 path_channel_id 是可行的
#ifdef __APPLE__
