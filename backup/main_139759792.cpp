...
//            for (int channel_id = 0; channel_id < num_of_channels; ++channel_id) {
//                path_nodes = bfs_find_path_nodes(nid_source, nid_target, channel_id);
//                if (!path_nodes.empty()) {
//                    path_edges_total = get_path_edges_total_via_nodes(path_nodes);
//                    is_channel_id_ok(path_edges_total, channel_id, path_edges);// should be true
//                    path_channel_id = channel_id;
//                    break;
//                }
//            }
...
#ifdef __APPLE__
        cerr << "path_edges.size() + 1 != path_nodes.size()" << endl;
#endif
    }
