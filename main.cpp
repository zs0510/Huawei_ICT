...
const double time_constraint = 100.f;// 当运行时间小于此值时, 允许重新铺设线路
double time_single_iteration = 0.f;// 记录单次迭代花费的最大时间
...
inline double how_many_seconds() {
...
inline bool compare_vector(vector<int>& vec0 , vector<int>& vec1) {
    int i = 0, j = 0;
    while (i < vec0.size() && j < vec1.size()) {
        if (vec0[i] != vec1[j]) {
            return vec0[i] < vec1[j];
        }
        ++i;
        ++j;
    }
    return i != vec0.size();// 前缀相同的情况下, 把长的放在前面
};
...
//        }
    ...
    // 处理
    while (true) {
        double time_iter_beg = how_many_seconds();
        clear_data();// 清除上次铺设的线路

        sort_data();// 对业务进行排序

        // TODO: 随机生成每个业务尝试通道的顺序
        if (pave()) break;// 铺设线路
        double time_iter_end = how_many_seconds();
        // 记录单次迭代花费的最大时间
        time_single_iteration = max(time_single_iteration, time_iter_end - time_iter_beg);
...
//    random_shuffle(operations.begin(), operations.end());
    ...
    vector<vector<int>> path_degree(num_of_operations);// 记录这个路径经过的边的度
    ...
    for (int op_id = 0; op_id < num_of_operations; ++op_id) {
        ...
        for (int i = 1; i < path_nodes.size(); ++i) {
            ...
            path_degree[op_id].push_back(cnt);
        }
    }

    for (auto& pd : path_degree) {
...
//        return compare_vector(path_degree[op0.id], path_degree[op1.id]);
    });
...
#ifdef __APPLE__
    cout << ", cost_cheapest = " << cost_cheapest << ": ";
    for (int i = 0; i < path_nodes_cheapest.size(); ++i) {
        cout << path_nodes_cheapest[i] << " ";
    }
    cout << endl;
#endif
...
bool pave() {
    ...
    for (auto& op : operations) {
       ...
       for (int channel_id = 0; channel_id < num_of_channels; ++channel_id) {
        ...
        // for (int channel_id = num_of_channels - 1; channel_id >= 0; --channel_id) {
        //     if (is_channel_id_ok(path_edges_total, channel_id, path_edges)) {
        //         path_channel_id = channel_id;
        //         break;
        //     }
...
//                }
                ...
                if (how_many_seconds() + time_single_iteration * 5 < time_constraint) {// 程序执行时间少于 45s, 重新优化
                    return false;
                }
